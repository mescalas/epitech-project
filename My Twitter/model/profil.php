...
class Info_profil extends MyDatabase
{
    ...
    public function get_user_tweet( $token )
    {
        ...
        IF(GROUP_CONCAT(like_tweet.`id_user`) LIKE (CONCAT('%', (SELECT user.id_user FROM user WHERE token LIKE :token), '%')) , 'oui', 'non') as 'user_like', Null AS 'user_retweet'
        FROM `tweet`
        ...
        IF(GROUP_CONCAT(retweet.`id_user`) LIKE (CONCAT('%', (SELECT user.id_user FROM user WHERE token LIKE :token), '%')) , 'oui', 'non') AS 'user_retweet'
        FROM `tweet`
        INNER JOIN user ON tweet.id_user = user.id_user
    ...
    {
        $sql = $this->pdo->prepare( "SELECT user.username, user.arobase, reply.contenu, reply.date_reply FROM reply INNER JOIN user ON user.id_user = reply.id_user WHERE id_post= :id_post" );
    ...
    public function reply_final( $id_post, $token )
    {
        try {
            ...
            $sql->execute();
            $id_user = $sql->fetch();
            $sql_    = $this->pdo->prepare( "SELECT user.username, user.arobase, reply.contenu, reply.date_reply FROM reply INNER JOIN user ON user.id_user = reply.id_user WHERE reply.id_user = :id_user AND reply.id_post = :id_post ORDER BY reply.date_reply DESC LIMIT 1" );
            $sql_->bindParam( ':id_user', $id_user[0] );
            $sql_->bindParam( ':id_post', $id_post );
            $sql_->execute();
            $result = $sql_->fetch();
